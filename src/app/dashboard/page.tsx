...
// Use internal API routes instead of external indexer
const INDEXER_API = typeof window !== 'undefined' ? window.location.origin : ''
  ...
  useEffect(() => {
    ...
    const checkConnection = async () => {
      try {
        ...
        const apiUrl = INDEXER_API || (typeof window !== 'undefined' ? window.location.origin : '')
      ...
      {indexerAvailable === false && (
        <div style={{
          ...
          <p style={{ marginTop: '0.5rem', marginBottom: 0, fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
            The indexer API is not available. The dashboard will show static information below.
          </p>
        </div>
      )}
